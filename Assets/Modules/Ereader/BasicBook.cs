...
namespace Ereader {
    ...
    public class BasicBook : Book {
        ...
        #region Building and Initializing

        public BasicBook(string title) {
            this.title = title;

            lines = File.ReadAllLines(Config.Instance.BookLibraryPath + "/VReader/" + title + "/" + title + ".txt");
            pages = new List<Page>();
        ...
        public override void Display() {
            ...
            nextPageLeft = PageIndLeft + 2 < GetPages().Count && PageIndLeft + 2 >= 0
                ? GetPages()[pageIndLeft + 2]
                : null;
            nextPageRight = PageIndRight + 2 < GetPages().Count && PageIndRight + 2 >= 0
                ? GetPages()[pageIndRight + 2]
                : null;
