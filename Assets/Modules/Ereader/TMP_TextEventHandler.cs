...
namespace Ereader
{
    ...
    {        
        ...
        [Serializable]
        public class WordMouseHoldClickEvent : UnityEvent<string, int, int> { }
        ...
        /// <summary>
        /// Event delegate triggered when mouse click is held for a longer duration
        /// </summary>
        public WordMouseHoldClickEvent onMouseHoldClick
        {
            get { return m_OnMouseHoldClick; }
            set { m_OnMouseHoldClick = value; }
        }
        [SerializeField]
        private WordMouseHoldClickEvent m_OnMouseHoldClick = new WordMouseHoldClickEvent();
        ...
        void LateUpdate()
        {
            if (TMP_TextUtilities.IsIntersectingRectTransform(m_TextComponent.rectTransform, Input.mousePosition, m_Camera)){
                ...
                else if (wordIndex != -1 && Input.GetMouseButtonUp(0)){
                    float duration = Time.time - mouseDownTime;
                    if (duration >= 1){
                        TMP_WordInfo wInfo = m_TextComponent.textInfo.wordInfo[wordIndex];
                        
                        StringBuilder sb = new StringBuilder(); // todo only one word in focus at a time. remove previous highlight.
                        sb.Append(m_TextComponent.text.Substring(0, wInfo.firstCharacterIndex));
                        sb.Append("<mark=#FFFF0080>");
                        sb.Append(m_TextComponent.text.Substring(wInfo.firstCharacterIndex, wInfo.characterCount));
                        sb.Append("</mark>");
                        sb.Append(m_TextComponent.text.Substring(wInfo.firstCharacterIndex + wInfo.characterCount));
                        m_TextComponent.text = sb.ToString();
                        
                        SendOnMouseHoldClickEvent(wInfo.GetWord(), wInfo.firstCharacterIndex, wInfo.characterCount);
                    }
                }
        ...
        private void SendOnMouseHoldClickEvent(string word, int charIndex, int length)
        {
            if (onMouseHoldClick != null)
                onMouseHoldClick.Invoke(word, charIndex, length);
        }
    }
}
